...
/* ─────────────────────────────────────────
   COUNTDOWN — autoejecutado, independiente
   Patrón probado: getTime() + setInterval
   Colombia = UTC-5 → 22 Aug 17:00 = UTC 22:00
───────────────────────────────────────── */
(function() {
  var second = 1000;
  ...
  // Fecha en UTC explícito — igual para todos los países
  var weddingDay = new Date(2026, 2, 22).getTime(); // 22 de marzo 2026
  ...
  function tick() {
    ...
    if (dEl) dEl.innerText = pad(days, 3);
    if (hEl) hEl.innerText = pad(hours);
    if (mEl) mEl.innerText = pad(minutes);
    if (sEl) sEl.innerText = pad(seconds);
  }

  // Corre inmediatamente y luego cada segundo
  tick();
  setInterval(tick, second);
})();

function initCountdown() { /* ya corre arriba, se mantiene por compatibilidad */ }
