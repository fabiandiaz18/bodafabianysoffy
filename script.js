...
/* ─────────────────────────────────────────
   COUNTDOWN
───────────────────────────────────────── */
const WEDDING_DATE  = new Date('2025-08-22T17:00:00');
const CIRCUMFERENCE = 2 * Math.PI * 34; // r = 34

function setUnit(id, display, value, max) {
  const numEl  = document.getElementById(`cd-${id}`);
  const ringEl = document.getElementById(`ring-${id}`);

  if (numEl) {
    const padded = id === 'days'
      ? String(display).padStart(3, '0')
      : String(display).padStart(2, '0');
    if (numEl.textContent !== padded) {
      numEl.style.transform = 'scale(1.18)';
      numEl.textContent = padded;
      setTimeout(() => { numEl.style.transform = ''; }, 130);
    }
  }

  if (ringEl) {
    const pct    = 1 - Math.min(value / max, 1);
...
function updateCountdown() {
  ...
  if (diff <= 0) {
    setUnit('days', 0, 0, 365);
    setUnit('hours', 0, 0, 24);
    setUnit('minutes', 0, 0, 60);
    setUnit('seconds', 0, 0, 60);
    return;
  }
  ...
  setUnit('days',    days,    days,    365);
  setUnit('hours',   hours,   hours,   24);
  setUnit('minutes', minutes, minutes, 60);
  setUnit('seconds', seconds, seconds, 60);
}
